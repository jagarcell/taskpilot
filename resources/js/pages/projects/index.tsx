...
const projectsPagePanelStorageKey = 'taskpilot-projects-panel-state';

interface ProjectsPagePanelState {
    showCreateProject?: boolean;
    showYourProjects?: boolean;
}

export function getProjectsPagePanelState(): ProjectsPagePanelState | null {
    ...
    try {
        const storedValue = storage.getItem(projectsPagePanelStorageKey);

        if (!storedValue) {
            return null;
        }

        return JSON.parse(storedValue) as ProjectsPagePanelState;
    } catch {
        return null;
    }
}

export function saveProjectsPagePanelState(state: ProjectsPagePanelState): void {
    const storage = typeof window !== 'undefined' ? window.localStorage : globalThis.localStorage;
    ...
    try {
        storage.setItem(projectsPagePanelStorageKey, JSON.stringify(state));
    } catch {
...
export default function ProjectsIndex({ projects }: ProjectsIndexProps) {
    const storedPanelState = getProjectsPagePanelState();
    const [showCreateProject, setShowCreateProject] = useState(storedPanelState?.showCreateProject ?? false);
    const [showYourProjects, setShowYourProjects] = useState(storedPanelState?.showYourProjects ?? false);

    useEffect(() => {
        saveProjectsPagePanelState({ showCreateProject, showYourProjects });
    }, [showCreateProject, showYourProjects]);

    return (
        <>
            <Head title="Projects" />
            <div className="mx-auto max-w-5xl px-3 py-8 sm:px-4 lg:px-6 min-w-[50%]">
                <div className="space-y-6">
                    <div className="mb-6 flex items-center justify-between gap-4">
                        <div>
                            <p className="text-sm font-medium uppercase tracking-[0.2em] text-sky-600">Projects</p>
                            <h1 className="mt-2 text-3xl font-semibold text-slate-900 dark:text-white">Projects</h1>
                        </div>
                        <Link
                            href={dashboard()}
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div className="mb-4 flex items-center justify-between gap-3">
                            ...
                            <button
                                type="button"
                                aria-label={showCreateProject ? 'Collapse create project' : 'Expand create project'}
                            ...
                            >
                                <svg
                                    ...
                                    stroke="currentColor"
                                    strokeWidth="1.8"
                                    className={`h-4 w-4 transition-transform ${showCreateProject ? 'rotate-180' : ''}`}
                                    aria-hidden="true"
                                >
                        ...
                        {showCreateProject ? (
                            <>
                                <p className="mb-4 text-sm text-slate-600 dark:text-slate-300">Create a new project to get started.</p>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                        <div className="mb-4 flex items-center justify-between gap-3">
                            ...
                            <button
                                type="button"
                                aria-label={showYourProjects ? 'Collapse your projects' : 'Expand your projects'}
                                onClick={() => setShowYourProjects((current) => !current)}
                                className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 transition hover:border-sky-200 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:border-sky-500/50 dark:hover:text-sky-300"
                            >
                                <svg
                                    viewBox="0 0 20 20"
                                    ...
                                    stroke="currentColor"
                                    strokeWidth="1.8"
                                    className={`h-4 w-4 transition-transform ${showYourProjects ? 'rotate-180' : ''}`}
                                    aria-hidden="true"
                                >
                                    <path d="M5 7.5L10 12.5L15 7.5" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
