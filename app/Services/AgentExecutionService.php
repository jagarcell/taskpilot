...
class AgentExecutionService
{
    ...
    public function execute(AgentRun $agentRun): AgentRun
    {
        ...
        try {
            ...
            if ($this->isPlanningAgent($agentRun)) {
                $this->persistPlanningArtifact($agentRun, $output);
            }
    ...
    /**
     * Persist the structured implementation plan generated by the planning agent so the approval stage has a concrete artifact to review.
     *
     * @param  AgentRun  $agentRun
     * @param  array<string, mixed>  $output
     * @return void
     * Logic: write a durable markdown plan artifact and attach the plan payload to the active workflow metadata so approval can be grounded in a real implementation outline instead of an empty agent run.
     */
    protected function persistPlanningArtifact(AgentRun $agentRun, array $output): void
    {
        $issue = $this->agentRunRepository->findIssueForRun($agentRun);
        ...
        $path = $directory.'/'.sprintf('planning-agent-issue-%d-%s.md', $issue->id, Str::slug((string) $issue->title ?: 'issue'));
        $plan = $output['plan'] ?? $output;
        $summary = is_string($output['summary'] ?? null) ? $output['summary'] : 'Implementation plan recorded for approval.';

        $content = "# Planning Agent Notes\n\n";
        ...
        foreach ($plan as $key => $value) {
            ...
            $content .= "### ".str_replace('_', ' ', (string) $key)."\n\n";
            if (is_array($value)) {
        ...
        $metadata['planning'] = array_merge($metadata['planning'] ?? [], [
            'summary' => $summary,
            'artifacts' => [$path],
            'plan' => $plan,
     ...
     */
    protected function isPlanningAgent(AgentRun $agentRun): bool
    {
        $agentName = strtolower((string) $agentRun->agent()->value('name'));

        return str_contains($agentName, 'planning');
    }
